...
public class Premium extends Consumer{
    ...
    public boolean create(String title) {
        return playlists.add(new Playlist(title));
    }
    ...
    public boolean addAudioContentToPlaylist(int indexPlaylist, AudioContent audioContent) {
        return playlists.get(indexPlaylist).add(audioContent);
    }
    ...
    public boolean removeAudioContentToPlaylist(int indexPlaylist, int indexAudioContent) {
        return playlists.get(indexPlaylist).remove(indexAudioContent);
    }
    ...
    public String showPlaylists() {
        ...
        for (int i = 0; i < playlists.size(); i++) {
            message += " (" + (i+1) + ") " + playlists.get(i).getTitle() + "\n";
        }
    ...
    public String sharePlaylist(int index) {
        return playlists.get(index).codeToSharing();
    }
    ...
    public String playPlaylist(int indexPlaylist, int action) {
        ...
        switch (action) {
            case 1:
                if (playlists.get(indexPlaylist).getLastContent().split(":")[1].equals("0")) message = playlists.get(indexPlaylist).playAudioContent(0);
                else if (playlists.get(indexPlaylist).getPlaybackStatus()) message = "Playing";
                else if (Integer.parseInt(playlists.get(indexPlaylist).getLastContent().split(":")[1]) > 0) {
                    if (Integer.parseInt(playlists.get(indexPlaylist).getLastContent().split(":")[1]) == 1) message = playlists.get(indexPlaylist).playAudioContent(0);
                    else message = playlists.get(indexPlaylist).playAudioContent(Integer.parseInt(playlists.get(indexPlaylist).getLastContent().split("-")[0]));
                    playlists.get(indexPlaylist).deleteUpdateLastContent();
                }
                break;
            case 2:
                if (playlists.get(indexPlaylist).getPlaybackStatus()) {
                    playlists.get(indexPlaylist).stop();
                    message = "Stopped";
                } else message = "It's stopped";
                break;
            case 3:
                message = playlists.get(indexPlaylist).playAudioContent(playlists.get(indexPlaylist).previous());
                break;
            case 4:
                message = playlists.get(indexPlaylist).playAudioContent(playlists.get(indexPlaylist).next());
                break;
            default:
    ...
    public boolean buy(Song song) {
        return purchases.add(new Purchase(song));
    }
    ...
    public String getPurchase() {
        ...
        for (int i = 0; i < purchases.size(); i++) {
            message += "  (" + (i+1) + ") " + purchases.get(i).toString() + "\n";
        }
        return message;
    }
